#!/usr/bin/env python3
"""
Evaluator2 for Qdrant baseline.

This evaluator mirrors the logic used in RAG/evaluation/evaluator.py by
matching retrieved results to ground truth passages via cosine similarity
and computing standard IR metrics (recall, precision, MRR, NDCG) at
multiple cutoffs. It reads the baseline retrieval outputs produced by
qdrant_retrieval.py and ground-truth queries from requests/final_requests.json,
then writes aggregated metrics to qdrant_result/evaluation_results.json using
the same JSON format as RAG/evaluation_results.json.
"""
...
try:
    ...
    logger.warning("Falling back to simple cosine matcher due to import error: %s", exc)
...
class Evaluator2:
    ...
    def _init_embedder(self):
        """
        Initialize the same embedder used in RAG (UniXcoder default).
        If import fails, we fall back to None and will short-circuit matches.
        """
        if UniXcoderEmbedder is None:
            logger.warning("UniXcoderEmbedder unavailable; similarity will degrade.")
            return None
        try:
...
def main():
    ...
    output_path = script_dir / ".." / ".." / "RAG" / "results" / "baseline_qdrant_results.json"
